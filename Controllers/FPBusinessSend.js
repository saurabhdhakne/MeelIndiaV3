  ...
  try {
    ...
    await mongoose.connect(url, async (err, db) => {
      if (err) {
        ...
        await business.find({ email: req.body.email }, (err, result) => {
          if (err) throw err;
          if (result.length > 0) {
            flag = true;
          } else {
            ...
            function (err, res) {
              ...
              nodemailer.createTestAccount((err, account) => {
                let transporter = nodemailer.createTransport({
                  ...
                  auth: {
                    user: `${process.env.email}`, //Gmail usename
                ...
                let mailOptions = {
                  ...
                  <h2> <a href='${process.env.ip}/FPBusinessReset?enc=${randomString}'> <strong> Click Here </strong> </a> <br> to Reset Password, Please do not share your password with anyone else.</h2> <br><br> 
